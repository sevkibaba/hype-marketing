...
class IntentDetector:
    ...
    def extract_product_info(self, message):
        ...
        # Platform and Sector detection
        platforms = []
        if any(word in message.lower() for word in ['mobil', 'ios', 'android', 'app', 'iphone', 'play store']):
            platforms.append('Mobil')
        if any(word in message.lower() for word in ['web', 'saas', 'browser', 'site']):
            platforms.append('Web')
        if any(word in message.lower() for word in ['dış ticaret', 'ihracat', 'ithalat', 'b2b', 'trade', 'yurtdışı']):
            platforms.append('Dış Ticaret')
...
class AnalysisPipeline:
    ...
    async def _analyze_persona(self, data):
        ...
        Sector/Platform: {product.get('platform', product.get('platforms', ['Unknown']))}
        Target: {audience.get('primary', 'Unknown')}
        
        If the sector is "Dış Ticaret" (Foreign Trade), focus on B2B buyers, importers, and regional distribution networks instead of typical individual users.
        
        Create 3 distinct personas with:
        1. Name and demographics (or Company Profile for B2B)
        2. Pain points
        3. Goals and motivations
        4. Where they spend time online (or which B2B portals they use)
    ...
    async def _research_keywords(self, data):
        ...
        prompt = f"""
        Generate Marketing Keywords / App Store (ASO) Optimization for this product:
        Name: {product['name']}
        Description: {product['description']}
        Sector/Platform: {product.get('platform', product.get('platforms', ['Unknown']))}
        
        If it's 'Dış Ticaret', generate B2B SEO keywords, import/export search queries, and B2B directory tags instead of App Store keywords.
        Otherwise, generate ASO/SEO keywords:
        
        Provide:
        1. 5 head terms (high volume, competitive)
        2. 10 long-tail keywords (specific intent)
        3. 5 competitor brand names for conquesting
        4. 5 feature-based or product-based keywords
